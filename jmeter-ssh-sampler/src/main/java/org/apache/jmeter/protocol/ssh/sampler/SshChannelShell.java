...
/**
 * This class is used to store the information about a persistent Shell channel
 * that is established over a persistent SSH session together with its input and
 * output stream
 */
public class SshChannelShell {
	...
	public byte[] readResponse(boolean stripCommand, String command, boolean stripPrompt, String encoding)
			throws Exception {
		String result = new String(readResponseBytes(), encoding);
		if (stripCommand == true) {
			// remove command if at first position and immediately followed by
			// CR/LF LF/Cr CR LF
		...
		// TODO handle special case command with empty response just prompt
		// returned
		if (stripPrompt) {
		...
		while (count > 0) {
			try {
				while (in.available() > 0) {
					...
					result = result2;
					// log.debug("while: "+Integer.toString(i)+"==="+new
					// String(tmp, "UTF-8")+"\n=====");
				}
			} catch (Exception e) {
				// log.info("(while)Exception in
				// SSHChannelShell:"+e.getMessage());
			...
			if (cShell.isClosed()) {
				try {
					...
					// log.info("(if closed)Exception in
					// SSHChannelShell:"+e.getMessage());
					return result;// TODO throw exception
				}
				count = 5;
			...
			try {
				...
				// log.info("(thread sleep)Exception in
				// SSHChannelShell:"+e.getMessage());
				return result;// TODO throw exception
			}
		}
