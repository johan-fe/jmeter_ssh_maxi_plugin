...
public enum GlobalDataSsh {
	...
	// if blank connection name is given the full list of connections is
	// returned
	// Retrieval operations (including get) generally do not block, so may
	// overlap with update operations
	// (including put and remove). Retrievals reflect the results of the most
	// recently completed
	// update operations holding upon their onset. For aggregate operations such
	// as putAll and clear,
	// concurrent retrievals may reflect insertion or removal of only some
	// entries.
	...
	// They do not throw ConcurrentModificationException. However, iterators are
	// designed to
	// be used by only one thread at a time.
	// ConcurrentHashMap achieves higher concurrency by slightly relaxing the
	// promises it makes to callers.
	// A retrieval operation will return the value inserted by the most recent
	// completed insert operation,
	// and may also return a value added by an insertion operation that is
	// concurrently in progress
	// (but in no case will it return a nonsense result). Iterators returned by
	// ConcurrentHashMap.iterator()
	// will return each element once at most and will not ever throw
	// ConcurrentModificationException,
	// but may or may not reflect insertions or removals that occurred since the
	// iterator was
	// constructed. No table-wide locking is needed (or even possible) to
	// provide thread-safety
	// when iterating the collection. ConcurrentHashMap may be used as a
	// replacement for synchronizedMap
	// or Hashtable in any application that does not rely on the ability to lock
	// the entire table to prevent updates
